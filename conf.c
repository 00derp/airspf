  ...
  while(fgets(conf_line, CONF_MAX_LEN, conf_file) != NULL){
	...
	*ptr = 0;
	...
	if(head->ignore == NULL){
      ...
      // CONF becomes inactive when stop is matched
      else if(strcmp(command, "stop") == 0){
	// the regexp from which we stop
        strncpy(head->stop, argument, sizeof(head->stop));
        head->stop_len = strlen(head->stop);
        printf("Stop detected: '%.*s'\n", head->stop_len, head->stop);
      }
